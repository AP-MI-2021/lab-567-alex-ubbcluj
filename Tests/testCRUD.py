...
def test_adauga_obiect():
    lst = []
    lst = adauga_obiect(33, "Frigider Samsung", "Clasa energetica A+, volum 384L", 1949, "C004", lst)
    lst = adauga_obiect(256, "Monitor Dell", "23.8'', Full HD, 144Hz", 1099, "C503", lst)
    assert len(lst) == 2
    assert get_ID(lst[1]) == 256
    assert get_nume(lst[1]) == "Monitor Dell"
    assert get_descriere(lst[1]) == "23.8'', Full HD, 144Hz"
    assert get_pret_achizitie(lst[1]) == 1099
...
def test_get_by_ID():
    ...
    assert get_pret_achizitie(get_by_ID(256, lst)) == 1099
...
def test_sterge_obiect():
    ...
    assert get_pret_achizitie(lst[0]) == 1099
...
def test_modifica_obiect():
    lst = []
    lst = adauga_obiect(33, "Frigider Samsung", "Clasa energetica A+, volum 384L", 1949, "C004", lst)
    lst = modifica_obiect(33, "Monitor Dell", "23.8'', Full HD, 144Hz", 1099, "C503", lst)
